...
namespace camellia {
    ...
    const action_timeline_keyframe_data *action_timeline_keyframe::get_data() const {
        return _data;
    }
    ...
    void action_timeline_keyframe::init(const action_timeline_keyframe_data *data,
...
#define CLASS_NAME "action_timeline"
    ...
    number_t action_timeline::get_effective_duration() const {
        ...
        return _p_data->effective_duration;
    }

    void action_timeline::init(const action_timeline_data &data, stage &stage, timeline_evaluator *p_parent) {
        _p_data = &data;
        _p_stage = &stage;
        _p_timeline_evaluator = p_parent;

        for (int t = 0; t < data.tracks.size(); t++) {
            auto &track = data.tracks[t];
            ...
            for (int i = 0; i < track.keyframes.size(); i++) {
                ...
                live_keyframe.init(&keyframe,
                                   this,
                                   t,
                                   i,
                                   (i >= track.keyframes.size() - 1 ? preferred_duration : std::min(
                                           track.keyframes[i + 1].time - keyframe.time, preferred_duration)));
            }
    ...
    std::vector<const action_timeline_keyframe *> action_timeline::sample(number_t timeline_time) const {
        ...
        for (auto &track: _tracks) {
            auto index = algorithm_helper::upper_bound<action_timeline_keyframe>(track,
                                                                                 [&](const action_timeline_keyframe &k) {
                                                                                     return algorithm_helper::compare_to(
                                                                                             k.get_time(),
                                                                                             timeline_time);
                                                                                 });
            if (index <= 0) continue;
    ...
    action_timeline::update(const number_t timeline_time, const boolean_t continuous) {
        std::vector<const action_timeline_keyframe *> res;
        ...
        if (!continuous) {
            ...
            for (int i = 0; i < _tracks.size(); i++) {
                ...
                while (_next_keyframe_indices[i] < track.size()
                       && (k = &track[_next_keyframe_indices[i]])->get_time() <= timeline_time) {
                    if (k->get_action().get_type() < 0) res.push_back(k);      // take instant actions into account
    ...
    variant action_timeline::get_base_value(const number_t timeline_time,
                                            const hash_t h_attribute_name,
    ...
    variant action_timeline::get_prev_value(const modifier_action &ac) const
    {
        ...
        if (ac.get_index() <= 0) return base_value;

        auto &last_keyframe = track[ac.get_index() - 1];
        auto &lac = last_keyframe.get_action();
        if (lac.get_type() != action_data::action_types::ACTION_MODIFIER) return base_value;
        const auto lma = dynamic_cast<modifier_action *>(&lac);
        if (lma->get_attribute_name_hash() != ac.get_attribute_name_hash()) return base_value;

        if (lma->final_value.get_value_type() == variant::VOID)
            lma->final_value = lma->apply_modifier(current_keyframe.get_time() - last_keyframe.get_time(),
                                                   ac.get_attribute_name_hash(),
                                                   base_value);
