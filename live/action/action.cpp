...
namespace camellia {
    ...
    void action::collect_action(const action *action) {
        delete action;
    }

    integer_t action::get_track_index() const {
        return track_index;
    }

    integer_t action::get_index() const {
        return index;
    }

    void action::init(const action_data *data,
    ...
    action_data::action_types modifier_action::get_type() const {
        return action_data::action_types::ACTION_MODIFIER;
    }

    const modifier_action_data *modifier_action::get_data() const {
        return dynamic_cast<const modifier_action_data *>(p_base_data);
    }

    hash_t modifier_action::get_name_hash() const {
        return get_data()->h_action_name;
    }
    ...
    hash_t modifier_action::get_attribute_name_hash() const {
        return get_data()->h_attribute_name;
    }

    variant::types modifier_action::get_value_type() const {
        return get_data()->value_type;
    }

    const std::map<text_t, variant> & modifier_action::get_default_params() const {
        return get_data()->default_params;
    }

    void modifier_action::init(const action_data *data,
                               action_timeline_keyframe *p_parent,
                               const integer_t ti,
                               const integer_t i) {
        ...
        auto mad = dynamic_cast<const modifier_action_data *>(data);
        ...
        try {
            ...
            throw std::runtime_error(
                    std::format("Error while evaluating modifier action script.\n"
                                "Script = {}, Action = {}\n"
                                "{}",
                                mad->h_script_name, mad->h_action_name,
                                err.what()));
    ...
    {
        if (get_attribute_name_hash() != h_attribute_name) return val;
    ...
    {
        if (!_is_valid) return base_value;
        ...
        try {
            ...
            throw std::runtime_error(std::format("Error while invoking function 'run()' of the modifier action script.\n"
                                                 "Script = {}, Action = {}\n"
                                                 "{}",
                                                 data->h_script_name, data->h_action_name,
                                                 err.what()));
        }
    }
}